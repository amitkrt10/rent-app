...
if "login" in st.session_state.keys():
    with st.spinner(text='Reading Data... Please Wait...!'):
        ...
        bankDf_j, totalDeposite_j, totalWithdraw_j, rentCollection_j, electricityExpense_j, wifiExpense_j, travelDeposite_j = st.session_state["bankDf"], st.session_state["totalDeposite"], st.session_state["totalWithdraw"], st.session_state["rentCollection"], st.session_state["electricityExpense"], st.session_state["wifiExpense"], st.session_state["travelDeposite"]
        st.balloons()
    ...
    with st.expander("HDFC Bank Statement"):
        #Show Table
        lendf = len(bankDf)
