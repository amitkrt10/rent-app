...
if "login" in st.session_state.keys():
    ...
    with st.expander("View Bills"):
        ...
        if selectedFlat == "Select":
            ...
            try:
                ...
                st.write(f"Your total rent due for the month of {selectMonth} is ₹ {viewList[8]}. Please pay the rent before 06/{viewList[0].strftime('%m/%Y')}. You can now view your bills and statements at https://tinyurl.com/kb-tenant . Your username is {userId} and password is {pw}")
            except:
    ...
    st.markdown("[Login](http://localhost:8501)")